...
customerRouter.post('/allcustomers', async (req, res, next) => {
    try {
        misQueryMod("Select * from magodmis.cust_data order by Cust_name asc", (err, data) => {
...
customerRouter.post('/allcustcodename', async (req, res, next) => {
    try {
        misQueryMod("Select Cust_Code,Cust_name from magodmis.cust_data order by Cust_name asc", (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post('/getcustomerdetails', async (req, res, next) => {
    try {
        ...
        misQueryMod(`Select * from magodmis.cust_data where Cust_Code='${custid}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
    ...
    try {
        ...
        misQueryMod(`Select * from magodmis.cust_data where lower(Cust_name)=Lower('${customerName}')`, (err, result) => {
            ...
            if (result.length > 0) {
                ...
                misQueryMod("Update magodmis.cust_data set IsBranch='" + isBranch + "',Branch='" + branchName + "' where Cust_name='" + customerName + "' And Cust_Code ='" + result["Cust_Code"] + "'", (err, result1) => {
                    if (err) logger.error(err);
                    res.send(result1);
                })
            }
                ...
                setupQuery("SELECT *  FROM magod_setup.magod_runningno WHERE SrlType='CustCode' ORDER BY Id DESC LIMIT 1;", async (runningno) => {
                    ...
                    misQueryMod(`Insert into magodmis.cust_data (Cust_Code,IsBranch,Cust_name,Branch) values ('${qno}', '${isBranch}','${customerName}','${branchName}')`, (err, ins) => {
                        if (err) logger.error(err);
                        console.log(ins)
                        if (ins && ins.affectedRows > 0) {
                            setupQuery(`UPDATE magod_setup.magod_runningno SET Running_No = Running_No + 1 WHERE SrlType='CustCode' And Id = ${runningno[0]["Id"]}`, async (updatedrunning) => {
...
customerRouter.post(`/customerupdate`, async (req, res, next) => {
    ...
    try {
        ...
        const maxcredit = (req.body.maxcredit == null) ? 0.00 : req.body.maxcredit;
        const creditdays = (req.body.creditdays == null) ? 0 : req.body.creditdays;
        const avepaydays = (req.body.avepaydays == null || req.body.avepaydays == '') ? 0 : req.body.avepaydays;
        ...
        console.log("cont details from req : " + JSON.stringify(req.body.custContactData))
                ...
                misQuery(`Update magodmis.cust_data set Branch = '${branchname}', Address='${custaddress}',City = '${custcity}', StateId= '${stateid}', State = '${state}', Country = '${country}',Pin_Code='${pincode}', EMail='${email}',IsGovtOrg=${govtorg},IsForiegn=${isexport}, GST_Exempt='${gstexempt}',CreditTerms='${crterm}',CreditTime='${creditdays}',CreditLimit=${maxcredit}, CreditTime=${creditdays}, AveragePymtPeriod=${avepaydays}, GSTNo='${gstno}',PAN_No='${panno}',DWG='${foldername}',CURRENT=${ccurrent},Delivery='${delivery}',CustStatus='OK' where Cust_Code='${custcode}'`,
                    async (response2) => {
                        ...
                        for (let i = 0; i < contactdetails.length; i++) {
                            console.log("inside For loop " + JSON.stringify(contactdetails))
                            // if (contactdetails[i]["conName"] === "") {
                            //     break;
                            // }
                            await misQueryMod(`Delete from magodmis.cust_contacts where Cust_code ='${custcode}'`, (err, deldata) => { if (err) logger.error(err); })
                            await misQueryMod(`Select * from magodmis.cust_contacts where Name = '${contactdetails[i].conName}' and Cust_code ='${custcode}'`, (err, contdata) => {
                                if (err) logger.error(err);
                                console.log(" contact exists " + contdata.length)
                                if (contdata.length > 0) {
                                    misQueryMod(`Update magodmis.cust_contacts set Designation = '${contactdetails[i].conDesignation}',E_mail ='${contactdetails[i].conE_mail}',
                                                Dept='${contactdetails[i].conDept}', Tele_Office='${contactdetails[i].conTele_Office}', Tele_Mobile='${contactdetails[i].conTele_Mobile}' 
                                                where Cust_code='${custcode}' and Name = '${contactdetails[i]["conName"]}'`, async (err, updata) => {
                                        if (err) logger.error(err);
                                    ...
                                    misQueryMod("insert into magodmis.cust_contacts (Cust_code,Name,Designation,E_mail,Dept,Tele_Office,Tele_Mobile)" +
                                        " values('" + custcode + "','" + contactdetails[i].conName + "','" + contactdetails[i].conDesignation +
                                        "','" + contactdetails[i].conE_mail + "','" + contactdetails[i].conDept + "','" + contactdetails[i].conTele_Office +
                                        "','" + contactdetails[i].conTele_Mobile + "')", async (err, contins) => {
                                            if (err) logger.error(err);
...
customerRouter.post('/getcustomercontactdets', async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`Select ContactID, Name as conName, Designation as conDesignation, Dept as conDept, E_mail as conE_mail,Tele_Office as conTele_Office,
                    Tele_Mobile as conTele_Mobile from magodmis.cust_contacts where Cust_Code='${custid}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/customerassy`, async (req, res, next) => {
    ...
    try {
        const custcode = req.body.custcode;
        if (!custcode) res.send(createError.BadRequest())
        misQueryMod(`SELECT * FROM magodmis.cust_assy_data where Cust_Code = '${custcode}' order by MagodCode asc`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post('/customerinsassembly', async (req, res, next) => {
    ...
    try {
        ...
        const mtrlcost = ((req.body.mtrlcost == null || req.body.mtrlcost == '') ? 0 : req.body.mtrlcost);
        const jwcost = ((req.body.jwcost == null || req.body.jwcost == '') ? 0 : req.body.jwcost);
        const assystatus = 'Edit'; // req.body.assystatus;
        console.log(req.body);

        setupQuery("SELECT *  FROM magod_setup.magod_runningno WHERE SrlType='Cust_AssyList' ORDER BY Id DESC LIMIT 1;", async (runningno) => {

            let magodassmid = 'Assy' + (parseInt(runningno[0]["Running_No"]) + 1).toString().padStart(6, '0')
            console.log(magodassmid)


            await misQueryMod(`INSERT INTO magodmis.cust_assy_data ( Cust_code,MagodCode, AssyCust_PartId, AssyDescription,MtrlCost,JobWorkCost,Status) VALUES('${custcode}','${magodassmid}', '${assycustpartid}', '${assydescription}',${mtrlcost},${jwcost},'${assystatus}')`, (err, ins) => {
                if (err) logger.error(err);
                console.log(ins)
                if (ins.affectedRows == 1) {
                    setupQuery(`UPDATE magod_setup.magod_runningno SET Running_No = Running_No + 1 WHERE SrlType='Cust_AssyList' And Id = ${runningno[0]["Id"]}`, async (updatedrunning) => {
                        console.log(`Updated running No ${JSON.stringify(updatedrunning)}`)
                    })
                }
            })
...
customerRouter.post('/chkassydupl', async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT * FROM magodmis.cust_assy_data where Cust_code = '${custcode}' and AssyCust_PartId = '${assycustpartid}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            if (data.length > 0) {
...
customerRouter.post('/custbomparts', async (req, res, next) => {
    ...
    try {
        ...
        if (!custcode || !partid || !partdescription) res.send(createError.BadRequest())
        setupQuery(`SELECT * FROM magodmis.cust_bomlist where Cust_code = '${custcode}' and PartId = '${partid}'`, async (data) => {
            if (data.length == 0) {

                setupQuery("SELECT *  FROM magod_setup.magod_runningno WHERE SrlType='BOMList' ORDER BY Id DESC LIMIT 1;", async (runningno) => {

                    let magodpartid = 'BOM ' + (parseInt(runningno[0]["Running_No"]) + 1).toString().padStart(10, '0')
                    console.log(magodpartid)


                    misQueryMod(`INSERT INTO magodmis.cust_bomlist ( MagodPartId, Cust_code,PartId, PartDescription) VALUES('${magodpartid}', '${custcode}','${partid}', '${partdescription}')`, (err, ins) => {
                        if (err) logger.error(err);
                        console.log(ins)
                        if (ins.affectedRows == 1) {
                            setupQuery(`UPDATE magod_setup.magod_runningno SET Running_No = Running_No + 1 WHERE SrlType='BOMList' And Id = ${runningno[0]["Id"]}`, async (updatedrunning) => {
                                console.log(`Updated running No ${JSON.stringify(updatedrunning)}`)
                            })

                            misQueryMod(`Select * from magodmis.cust_bomlist where cust_code='${custcode}'`, (err, data) => {
                                if (err) logger.error(err);
...
customerRouter.post("/getcustomercontactdets", async (req, res, next) => {
    ...
    try {
        const { custcode } = req.body.custcode;
        misQueryMod(`SELECT * FROM magodmis.contact_data where Cust_Code = '${custcode}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/getcustomerbomparts`, async (req, res, next) => {
    ...
    try {
        ...
        console.log(ccode);
        misQueryMod(`SELECT * FROM magodmis.cust_bomlist where Cust_code ='${ccode}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
    ...
    try {
        ...
        for (let i = 0; i < dataarray.length; i++) {
         ...
         //   console.log(dataarray[i]);
            if ((dataarray[i].assyPartId != '') && ((dataarray[i].partid != '') && (dataarray[i].partid != null))) {
                console.log("Saving Assm Parts : "+dataarray[i].partid+"  cust : "+ccode);
                misQueryMod(`Insert into magodmis.cust_assy_bom_list (Cust_AssyId, Cust_BOM_ListId, Quantity)
            Values(
                ...
                (SELECT bomlist.Id FROM magodmis.cust_bomlist as bomlist where PartId = '${dataarray[i].partid}' and Cust_code='${ccode}'), ${dataarray[i].qty})`, (err, data) => {
                    if (err) logger.error(err);
                    retdata.push(data);
                })
            }
...
customerRouter.post("/custbomassemblyparts", async (req, res, next) => {
 //   console.log("Customer BOM Assembly Parts" + req.body.custcode)
    ...
    try {
        ...
        misQueryMod(`SELECT asm.AssyCust_PartId as assyPartId, bom.PartId as partid,bom.PartDescription as partdesc,asmbom.Quantity as qty from magodmis.cust_assy_data asm
        ...
        where asm.Cust_Code='${custcode}' and asm.AssyCust_PartId ='${assyId}'`, (err, data) => {
            if (err) logger.error(err);
    //        console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/getcustomer`, async (req, res, next) => {
  //   console.log("Cust Code received");
    ...
    //    console.log(custcode);
        ...
        if (!custcode) res.send(createError.BadRequest())
        misQueryMod(`SELECT * FROM magodmis.cust_data where Cust_Code = '${custcode}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
//    console.log(req.body);
    try {
        ...
        misQueryMod(`SELECT Dwg_Code,Cust_Code,DwgName,Mtrl_Code,DxfLoc,Operation,MtrlCost,JobWorkCost,LOC,Holes,FORMAT(Part_Wt,3) as Weight FROM magodmis.dwg_data where Cust_Code = '${custcode}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/customerorders`, async (req, res, next) => {
    ...
    try {
        ...
        if (ordtype == null) {
            if (ordstatus !== 'All') {
                misQueryMod(`SELECT o.*,c.Cust_name FROM magodmis.order_list o 
                left join magodmis.cust_data c on c.Cust_Code = o.Cust_Code
                WHERE o.Cust_Code='${custcode}' AND  Order_Status ='${ordstatus}'   ORDER BY o.Order_Date Desc`, (err, data) => {
                    if (err) logger.error(err);
                ...
                misQueryMod(`SELECT o.*,c.Cust_name FROM magodmis.order_list o 
                left join magodmis.cust_data c on c.Cust_Code = o.Cust_Code  
                WHERE o.Cust_Code='${custcode}' ORDER BY o.Order_Date Desc`, (err, data) => {
                    if (err) logger.error(err);
                    console.log(data)
                    res.send(data)
                })
            ...
            if (ordstatus != 'All') {
                misQueryMod(`SELECT o.*,c.Cust_name FROM magodmis.order_list o 
                left join magodmis.cust_data c on c.Cust_Code = o.Cust_Code
                WHERE o.Cust_Code='${custcode}' AND  Order_Status ='${ordstatus}' and Type='${ordtype}' And Order_Type='${ordertype}'  ORDER BY o.Order_Date Desc`, (err, data) => {
                    if (err) logger.error(err);
                    console.log(data)
                    res.send(data)
                })
...
customerRouter.post(`/orderstatus`, async (req, res, next) => {
    try {
        ...
        setupQuery(`SELECT * FROM magod_setup.magod_statuslist s where s.Function = 'Order' order by Seniority asc`, (data) => {
            res.send(data)
        })
    } catch (error) {
        next(error)
...
customerRouter.post(`/orderschedule`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT o.*  FROM magodmis.orderschedule o WHERE o.Order_No ='${orderno}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/orderschtasks`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT n.* FROM magodmis.nc_task_list n
        inner join magodmis.orderschedule o on o.ScheduleID = n.ScheduleID
        WHERE  o.Order_No ='${orderno}' and n.ScheduleID = '${ordschid}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/orderdetails`, async (req, res, next) => {
    ...
    try {
        const orderno = req.body.orderno;
        misQueryMod(`SELECT o.*  FROM magodmis.order_details o WHERE o.Order_No ='${orderno}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/orderinvoices`, async (req, res, next) => {
    try {
        console.log(req.body);
        const orderno = req.body.orderno;
        misQueryMod(`SELECT n.* FROM magodmis.draft_dc_inv_register n, magodmis.orderschedule o 
        WHERE n.ScheduleID=o.ScheduleID AND o.Order_No ='${orderno}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/orderinvdwg`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT * FROM magodmis.draft_dc_inv_details d WHERE d.DC_Inv_No= '${dcinvno}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/schdets`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT n.* FROM magodmis.orderscheduledetails n WHERE n.ScheduleID='${scheduleid}'`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/schtasksdets`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT t.* FROM magodmis.task_partslist t WHERE t.NcTaskId='${nctaskid}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
    } catch (error) {
...
customerRouter.post(`/printduereport`, async (req, res, next) => {
    ...
    try {
        ...
        and d.GrandTotal > d.PymtAmtRecd ) a`, (err, duestypedata) => {
                if (err) logger.error(err);
                console.log(" dues data " + JSON.stringify(duestypedata));

                misQueryMod(`SELECT DateDiff(Curdate(),d.Inv_Date) as DueDays,DC_Inv_No,IsDC,DATE_FORMAT(Dc_inv_Date, "%d/%l/%Y") AS 'Dc_inv_Date' ,DC_InvType,
                ...
                WHERE  d.DCStatus='Despatched' AND d.Cust_Code='${custcode}' ORDER BY d.Inv_Date`, (err, duedata) => {
                    if (err) logger.error(err);
...
customerRouter.post(`/customermtrlstock`, async (req, res, next) => {
    try {
        ...
        misQueryMod(`SELECT MtrlStockID, count( MtrlStockID) as inStock,Cust_Docu_No, Mtrl_Code, DynamicPara1, DynamicPara2,Locked, Scrap 
                    FROM magodmis.mtrlstocklist  WHERE Cust_Code='${custcode}' 
                GROUP BY Mtrl_Code, DynamicPara1, DynamicPara2,Scrap, Locked ORDER BY  Locked DESC,Scrap DESC`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/getmtrlrvlist`, async (req, res, next) => {
    try {
        ...
        if (ccode == '') {
            if (source == "Magod") {
                misQueryMod(`SELECT * FROM magodmis.material_receipt_register m WHERE m.Type='${type}' 
                        AND m.RVStatus='${status}' and m.Cust_Code like '0000' ORDER BY  m.RVId Desc`, (err, data) => {
                    if (err) logger.error(err);
                    res.send(data)
                })
            }
                ...
                misQueryMod(`SELECT * FROM magodmis.material_receipt_register m WHERE m.Type='${type}' 
                        AND m.RVStatus='${status}' and m.Cust_Code not like '0000' ORDER BY  m.RVId Desc`, (err, data) => {
                    if (err) logger.error(err);
                    res.send(data)
                })
            }
            ...
            if (source == "Magod") {
                ...
                misQueryMod(`SELECT * FROM magodmis.material_receipt_register m WHERE m.Type='${type}' 
                        AND m.RVStatus='${status}' and m.Cust_Code = '${ccode}' ORDER BY  m.RVId Desc`, (err, data) => {
                    if (err) logger.error(err);
                    res.send(data)
                })
            }
...
customerRouter.post(`/customermtrlreceipts`, async (req, res, next) => {
    try {
        ...
        misQueryMod(`SELECT RVID,RV_No, RV_Date,  CustDocuNo, RVStatus,TotalWeight,updated, TotalCalculatedWeight 
        FROM magodmis.material_receipt_register WHERE Cust_Code='${custcode}' ORDER BY RV_Date DESC,RV_no DESC`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/customermtrlrectdetails`, async (req, res, next) => {
    try {
        ...
        misQueryMod(`SELECT rvID, Mtrl_Code,DynamicPara1, DynamicPara2, Qty,updated FROM magodmis.mtrlreceiptdetails  WHERE rvID= '${rvid}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/updatebomassembly`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`Select * from magodmis.cust_assy_data where MagodCode='${mmagodid}'`, (err, data) => {
            if (err) logger.error(err);
            if (data.length > 0) {
                misQueryMod(`Update magodmis.cust_assy_data set Status='${asmstatus}',AssyDescription = '${asmdesc}',MtrlCost='${mtrlcost}',JobWorkCost='${jbwrkcost}' where MagodCode='${mmagodid}'`, (err, bomasmdata) => {
                    if (err) logger.error(err);
...
customerRouter.post(`/deletebomassmparts`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`Select assytbl.Id as assyid from magodmis.cust_assy_data assytbl where AssyCust_PartId = '${asmid}'`, (err, asmdata) => {
            if (err) logger.error(err);
            console.log(asmdata);
            console.log(asmdata[0].assyid)
            console.log(asmpart);
            misQueryMod(`SELECT bomlist.Id FROM magodmis.cust_bomlist as bomlist where PartId = '${asmpart}'`, (err, asmprtdata) => {
                if (err) logger.error(err);
                console.log("asmprtdata " + asmprtdata[0].Id)
                misQueryMod(`Delete from magodmis.cust_assy_bom_list  where Cust_AssyId ='${asmdata[0].assyid}' and Cust_Bom_ListId='${asmprtdata[0].Id}'`, (err, deldata) => {
                    if (err) logger.error(err);
                })
            })
        })
...
customerRouter.post(`/customermtrlscrapUnusedreturned`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT d.DC_No, d.DC_Date, d1.Material, sum(d1.DC_Srl_Wt) as Total_Wt FROM magodmis.dc_register d
        inner join magodmis.dc_details d1 on d1.DC_ID=d.DC_ID
                            WHERE d.DC_Type='Material Return' AND d.Cust_Code='${custcode}'
                            GROUP BY d1.Material, d.DC_No, d.DC_Date ORDER BY d.DC_Date , d.DC_No desc`, (err, data) => {
            if (err) logger.error(err);
...
customerRouter.post(`/customerduelist`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT DateDiff(Curdate(),d.Inv_Date) as DueDays,DC_Inv_No,IsDC,ScheduleId,DATE_FORMAT(Dc_inv_Date, "%d/%l/%Y") AS 'Dc_inv_Date' ,DC_InvType,
        ...
        WHERE  d.DCStatus='Despatched' AND d.Cust_Code='${custcode}' ORDER BY d.Inv_Date`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/customeroverduelist`, async (req, res, next) => {
    ...
    try {
        ...
        Com_inv_id, Remarks,PO_Value,FB_Qty,FB_Quality,FB_Delivery,FB_Remarks,Del_responsibility,
        PaymentTerms,pkngLevel,SummaryInvoice,BillType,Sync_HOId,PAN_No,ST_No FROM magodmis.draft_dc_inv_register d 
        WHERE  d.DCStatus='Despatched' AND d.Cust_Code='${custcode}' and DateDiff(Curdate(),d.Inv_Date) > '${custcr}'  ORDER BY d.Inv_Date`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/customerduesoverdues`, async (req, res, next) => {
    ...
    try {
        ...
        // and d.GrandTotal > d.PymtAmtRecd ) a`, (err, data) => {
            misQueryMod(`Select  case when DueDays > crdays then Sum(Balance) else 0 end as overDue,
...
customerRouter.post(`/customerdlinvformtaxdets`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT * FROM magodmis.dc_inv_taxtable d WHERE d.Dc_Inv_No='${dcinvno}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/customerduessummary`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`Select Sum(DueAmt30) as DueAmt30, Sum(DueAmt60) as DueAmt60, Sum(DueAmt90) as DueAmt90, Sum(DueAmt180) as DueAmt180, Sum(DueAmt365) as DueAmt365, Sum(DueAmtAbv365) as DueAmtAbv365 from
        (SELECT dd.Cust_Code,
            CASE
...
customerRouter.post(`/customeroutstandings`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`select ab.Cust_Code,ab.Cust_Name, Sum(DueAmt30 + DueAmt60 + DueAmt90 + DueAmt180 + DueAmt365 + DueAmtAbv365) as TotalDues,
        Sum(DueAmt30) as DueAmt30, Sum(DueAmt60) as DueAmt60, Sum(DueAmt90) as DueAmt90, 
       Sum(DueAmt180) as DueAmt180, Sum(DueAmt365) as DueAmt365, Sum(DueAmtAbv365) as DueAmtAbv365 from
       ...
       where (dd.PymtAmtRecd < dd.GrandTotal) ) ab
       where DueAmt30 > 0 or DueAmt60 >0 or DueAmt90 >0 or DueAmt180 > 0 or DueAmt365> 0 or DueAmtAbv365 > 0
       Group by Cust_Code Order by Cust_Name asc`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/outstandinginvoices`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT ddir.Inv_No,ddir.Inv_Date, ddir.GrandTotal, ddir.PymtAmtRecd, (ddir.GrandTotal - ddir.PymtAmtRecd) as Due, 
        ...
        WHERE  ddir.DCStatus ='Despatched' AND ddir.Cust_Code='${custcode}' ORDER BY ddir.Inv_Date`, (err, data) => {
            if (err) logger.error(err);
            console.log(data)
            res.send(data)
        })
...
customerRouter.post(`/customerreceiptsinfo`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT * FROM magodmis.payment_recd_voucher_register p WHERE p.Cust_code='${custcode}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
...
customerRouter.post(`/customerreceiptdets`, async (req, res, next) => {
    ...
    try {
        ...
        misQueryMod(`SELECT * FROM magodmis.payment_recd_voucher_details p WHERE p.RecdPVID='${recdpvid}'`, (err, data) => {
            if (err) logger.error(err);
            res.send(data)
        })
